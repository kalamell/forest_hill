...
<table id='data-grid' title="โปรโมชั่น" class="easyui-datagrid" url="<?=site_url('promotion/getdata');?>"
    ...
    <thead>
    <tr>
        <th field="promotion_id" width="10" hidden="true">ID</th>
        <th field="promotion_name" width="30">ชื่อโปรโมชั่น</th>
        <th field="promotion_price" width="30">มูลค่า</th>
    </tr>
    </thead>
</table>
    ...
    <?php echo form_open_multipart("",array("id"=>"fm"));?>
        ...
        <table class="table">
            <tbody>
                <tr>
                    <td>ชื่อโปรโมชั่น</td>
                    <td><input type="text" name="promotion_name" class='easyui-textbox' required="required" /></td>
                </tr>
                <tr>
                    <td>มูลค่า</td>
                    <td><input type="text" name="promotion_price" class='easyui-textbox' required="required" /></td>
                </tr>
            </tbody>
        </table>
    <?php echo form_close();?>
...
var edit = false;
function newItem(){
    $('#dlg').dialog('open').dialog('setTitle','โปรโมชั่น');
    $('#fm').form('clear');
    url = '<?=site_url('promotion/do_save');?>';
    edit =false;
};
...
function editItem(){
    ...
    if (row){
        ...
        $('#fm').form('load',row);
        url = '<?=site_url('promotion/do_update');?>';
        edit = true;
    }
    else
    {
...
function removeItem(){
    ...
    if (row){
        $.messager.confirm('Confirm','คุณต้องการลบหรือไม่ ?',function(r){
            if (r){
                $.post('<?=site_url('promotion/delete');?>' ,{ promotion_id:row.promotion_id },function(result){
                    if (result.success){
                        $('#data-grid').datagrid('reload');    // reload the user data
                    } else {
