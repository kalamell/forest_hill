...
<table id='data-grid' title="จัดการหน่วยงาน" class="easyui-datagrid" url="<?=site_url('department/getdata');?>"
    ...
    <thead>
    <tr>
        <th field="department_id" width="10" hidden="true">ID</th>
        <th field="department_name" width="30">ชื่อหน่วยงาน</th>
    </tr>
    </thead>
</table>
    ...
    <?php echo form_open_multipart("",array("id"=>"fm"));?>
        ...
        <table class="table">
            <tbody>
                <tr>
                    <td>ชื่อหน่วยงาน</td>
                    <td><input type="text" name="department_name" class='easyui-textbox' required="required" /></td>
                </tr>
            </tbody>
        </table>
    <?php echo form_close();?>
...
var edit = false;
function newItem(){
    $('#dlg').dialog('open').dialog('setTitle','จัดการหน่วยงาน');
    $('#fm').form('clear');
    url = '<?=site_url('department/do_save');?>';
    edit =false;
};
...
function editItem(){
    ...
    if (row){
        ...
        $('#fm').form('load',row);
        url = '<?=site_url('department/do_update');?>';
        edit = true;
    }
    else
    {
...
function removeItem(){
    ...
    if (row){
        $.messager.confirm('Confirm','คุณต้องการลบหรือไม่ ?',function(r){
            if (r){
                $.post('<?=site_url('department/delete');?>' ,{ department_id:row.department_id },function(result){
                    if (result.success){
                        $('#data-grid').datagrid('reload');    // reload the user data
                    } else {
