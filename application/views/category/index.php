...
<table id='data-grid' title="หมวดงาน" class="easyui-datagrid" url="<?=site_url('category/getdata');?>"
    ...
    <thead>
    <tr>
        <th field="category_id" width="10" hidden="true">ID</th>
    ...
    <?php echo form_open_multipart("",array("id"=>"fm"));?>
        ...
        <table class="table">
            <tbody>
                <tr>
                    <td>หมวดงาน</td>
                    <td><input type="text" name="category_name" class='easyui-textbox' required="required" /></td>
                </tr>
            </tbody>
        </table>
    <?php echo form_close();?>
...
function editItem(){
    ...
    if (row){
        ...
        $('#fm').form('load',row);
        url = '<?=site_url('category/do_update');?>';
        edit = true;
    }
    else
    {
...
function removeItem(){
    ...
    if (row){
        $.messager.confirm('Confirm','คุณต้องการลบหรือไม่ ?',function(r){
            if (r){
                $.post('<?=site_url('category/delete');?>' ,{ category_id:row.category_id },function(result){
                    if (result.success){
                        $('#data-grid').datagrid('reload');    // reload the user data
                    } else {
