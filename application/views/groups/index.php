...
<table id='data-grid' title="กลุ่มงาน" class="easyui-datagrid" url="<?=site_url('groups/getdata');?>"
    ...
    <thead>
    <tr>
        <th field="groups_id" width="10" hidden="true">ID</th>
...
<div id="dlg" class="easyui-dialog" closed="true" buttons="#dlg-buttons" data-options="modal: true, resizable: true">
    <?php echo form_open_multipart("",array("id"=>"fm"));?>
        <input type="hidden" name="groups_id" value=""/>
        <table class="table">
            <tbody>
                <tr>
                    <td>กลุ่มงาน</td>
                    <td><input type="text" name="groups_name" class='easyui-textbox' required="required" /></td>
                </tr>
            </tbody>
        </table>
    <?php echo form_close();?>
...
function removeItem(){
    ...
    if (row){
        $.messager.confirm('Confirm','คุณต้องการลบหรือไม่ ?',function(r){
            if (r){
                $.post('<?=site_url('groups/delete');?>' ,{ groups_id:row.groups_id },function(result){
                    if (result.success){
                        $('#data-grid').datagrid('reload');    // reload the user data
                    } else {
