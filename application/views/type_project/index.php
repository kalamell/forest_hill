...
<table id='data-grid' title="ประเภทโครงการ" class="easyui-datagrid" url="<?=site_url('type_project/getdata');?>"
    ...
    <thead>
    <tr>
        <th field="type_project_id" width="10" hidden="true">ID</th>
        <th field="type_project_name" width="30">ชื่อประเภทโครงการ</th>
    </tr>
    </thead>
</table>
...
<div id="dlg" class="easyui-dialog" closed="true" buttons="#dlg-buttons" data-options="modal: true, resizable: true">
    <?php echo form_open_multipart("",array("id"=>"fm"));?>
        <input type="hidden" name="type_project_id" value=""/>
        <table class="table">
            <tbody>
                <tr>
                    <td>ชื่อประเภทโครงการ</td>
                    <td><input type="text" name="type_project_name" class='easyui-textbox' required="required" /></td>
                </tr>
            </tbody>
        </table>
    <?php echo form_close();?>
...
var edit = false;
function newItem(){
    $('#dlg').dialog('open').dialog('setTitle','ประเภทโครงการ');
    $('#fm').form('clear');
    url = '<?=site_url('type_project/do_save');?>';
    edit =false;
};
...
function editItem(){
    ...
    if (row){
        ...
        $('#fm').form('load',row);
        url = '<?=site_url('type_project/do_update');?>';
        edit = true;
    }
    else
    {
...
function removeItem(){
    ...
    if (row){
        $.messager.confirm('Confirm','คุณต้องการลบหรือไม่ ?',function(r){
            if (r){
                $.post('<?=site_url('type_project/delete');?>' ,{ type_project_id:row.type_project_id },function(result){
                    if (result.success){
                        $('#data-grid').datagrid('reload');    // reload the user data
                    } else {
